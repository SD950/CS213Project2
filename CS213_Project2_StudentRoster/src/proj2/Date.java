...
/**
 * Date class that implements the Java Interface Comparable used to determine if
 * payment dates are valid. Any date with year before 2021 are considered
 * invalid
 * 
...
public class Date implements Comparable<Date> {
	...
	/**
	 * Used to create a Date Object with variables taken by a string parameter in a
	 * certain format.
	 * 
	...
	public boolean leapYearDateValid(Date date) {
		if ((date.month == FEBRUARY) && (date.day > 29 || date.day < 1)) {
			return false;
		} else if ((date.month == APRIL || date.month == JUNE || date.month == SEPTEMBER || date.month == NOVEMBER)
				&& (date.day > 30 || date.day < 1)) {
			return false;
		} else if ((date.month == JANUARY || date.month == MARCH || date.month == MAY || date.month == JULY
	...
	public boolean nonLeapYear(Date date) {
		if ((date.month == FEBRUARY) && (date.day > 28 || date.day < 1)) {
			return false;
		} else if ((date.month == APRIL || date.month == JUNE || date.month == SEPTEMBER || date.month == NOVEMBER)
				&& (date.day > 30 || date.day < 1)) {
			return false;
	...
	/**
	 * Method to see if a date is valid date to be used Date with the year less than
	 * 2021 or a date beyond today is invalid.
	 * 
